...
arr=()
for ((x=2714; x<3338; x++)); do
    arr+=($x)
done

# arr=()
# for ((x=2714; x<2715; x++)); do
#     arr+=($x)
# done
# (只在224上训练)
echo "预测结果保存路径：${dir}"
...
domain="globalscale"
if [[ "$OOD" == "OOD" ]]; then
    domain="global_scale_out_of_domain"
    arr=()
    for ((x=0; x<130; x++)); do
        arr+=($x)
    done
    echo "Running on the out-of-domain(OOD) dataset."
fi
...
function run_task() {
    ...
    # 执行任务
    python ./apls/convert.py "../data/"${domain}"/all/region_${i}_graph_gt.pickle" "${i}_gt.json" || { echo "convert.py failed for $i"; exit 1; }
    python ./apls/convert.py "../${dir}/decode_result/${i}_graph_${suffix}_tracing.p" "${i}_prop.json" || { echo "convert.py failed for $i"; exit 1; }
