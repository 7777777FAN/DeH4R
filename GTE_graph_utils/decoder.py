...
def graph_deduplication(neighbors):
    '''(deprecated, 过于严苛)可以去除 成loop和未成loop的压盖情况'''
    # 从某个度至少为2的点开始追踪两条同方向的连续边
    # 1. 不再有同方向的边（其中一条连续边无法再扩展或者方向不再满足条件）
    # 2. 两条连续变都到达同一个点（成环）
    
    def _remove_edges(removed_edges, edge_nodes):
        removed_edges.append((k, edge_nodes[1]))
        if len(edge_nodes) >= 3:
            for i in range(1, len(edge_nodes)-1):	# 去除首尾点
                removed_edges.append((edge_nodes[i], edge_nodes[i+1]))
                removed_edges.append((edge_nodes[i], edge_nodes[i-1]))
        removed_edges.append((edge_nodes[-1], edge_nodes[-2]))
        ...
        for nei1 in v:
            ...
            for nei2 in v:
                ...
                if nei2 in impact:
                    continue
                if neighbors_cos(neighbors, k, nei1, nei2) > 0.984:
                    edge_nodes_1.append(k)
                    edge_nodes_2.append(k)
                    edge_nodes_1.append(nei1)
                    edge_nodes_2.append(nei2)
                    len_1 += np.linalg.norm(np.array(k) - np.array(nei1))
                    len_2 += np.linalg.norm(np.array(k) - np.array(nei2))
                    impact.append(k)
                    
        if len(edge_nodes_1) and len(edge_nodes_2):
            reach_end_status = 0	# 1成环，2无法再扩展或方向始终不满足条件
            while 0 == reach_end_status:
                if (len(neighbors[edge_nodes_1[-1]])) == 1 or (len(neighbors[edge_nodes_2[-1]]) == 1):
                    reach_end_status = 2
                    break
                new_start = False	# 重启双for循环
                for nei1 in neighbors[edge_nodes_1[-1]]:
                    if nei1 in impact:
                        continue
                    if nei1 == k:
                        continue
                    for nei2 in neighbors[edge_nodes_2[-1]]:
                        if nei2 in impact:
                            continue
                        if nei2 == k:
                            continue
                        vec_1 = np.array(nei1) - np.array(edge_nodes_1[-1])
                        vec_2 = np.array(nei2) - np.array(edge_nodes_2[-1])
                        l1 = np.linalg.norm(vec_1)
                        l2 = np.linalg.norm(vec_2)
                        if nei1 == nei2:
                            edge_nodes_1.append(nei1)
                            edge_nodes_2.append(nei2)
                            len_1 += l1
                            ...
                            if np.dot(vec_1, vec_2) > 0.984:
                                ...
                                new_start = True
                                break
                    if reach_end_status or new_start:
                        break
                if not reach_end_status and not new_start: # 至少其中一条边无法再扩展或者方向始终不匹配
                    reach_end_status = 2
   
            if 1 == reach_end_status:     
                if len_1 <= len_2:	# 成环，保留短边
                    edge_nodes_1, edge_nodes_2 = edge_nodes_2, edge_nodes_1
            elif 2 == reach_end_status:	# 无法再扩展或方向始终不满足条件
                if len_1 > len_2:	# 保留长边
                    edge_nodes_1, edge_nodes_2 = edge_nodes_2, edge_nodes_1
            removed_edges = _remove_edges(removed_edges, edge_nodes_1)       
    ...
    for k, v in new_neighbors.items():  # 去除可能没删掉的边
        new_v = v
        for nei in v:
            if nei not in new_neighbors:
                new_v.remove(nei)
        new_neighbors[k] = new_v
    
    return new_neighbors, len(removed_edges)
