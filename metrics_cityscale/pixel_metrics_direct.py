...
def pred_graph_drawer(patch_name, args):
    pred_graph = pickle.load(open(f"../{args.savedir}/decode_result/{patch_name}_graph_{args.suffix}_tracing.p", 'rb'))
...
def cal_pixelscores(evaluator, name, thr=0.23):
    gt_path = os.path.join('../data/cityscale/road_mask/', f'region_{name}.png')
    pred_path = os.path.join(f'../{args.savedir}/pred_mask', f'{name}_pred_road.png')
    
    gt_mask = np.array(Image.open(gt_path)).astype(np.uint8)
    
    pred_mask = (np.array(Image.open(pred_path)) > 255 * thr).astype(np.uint8)
...
if '__main__' == __name__:
    ...
    args.suffix = 'with' if args.trace else 'no'
    ...
    with open(f"../{args.savedir}/score/pixelscore_direct.json", 'w') as f:
